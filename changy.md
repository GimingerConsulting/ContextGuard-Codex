...
- Deterministic mechanism benchmark: task evidence was 954 bytes (~239 estimated tokens), four structured summaries were 1,281 bytes, and the combined 2,235-byte payload was 73.98% smaller than the prior live orientation phase's 8,590 bytes. This proves the mechanism/output boundary, not that a stochastic Codex run will skip every prior command.
...
- Final release validation passed 207 tests, source compilation, plugin manifest validation and `git diff --check`. Release cachebuster: `0.9.0+codex.20260711085629`.
