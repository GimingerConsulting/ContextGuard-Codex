...
def test_compound_codex_shell_envelope_uses_selective_capture(tmp_path: Path):
    result = run_hook(
        "pre_tool_use.py",
        {
            "tool_name": "Bash",
...
def test_simple_codex_shell_envelope_is_rewritten(tmp_path: Path):
    result = run_hook(
        "pre_tool_use.py",
        {
            "tool_name": "Bash",
            "tool_input": {"command": "/bin/zsh -lc 'python3 -m pytest -q'"},
        },
        tmp_path,
    )
